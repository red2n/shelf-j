...
/// `storeql/notifications/{tenantId}/{storeId}`). This is a "wake up and refetch" signal plus
  ...
  Future<void> _connect(String tenantId, String jwt) async {
    final disconnect = await connectLiveAlerts(
      ...
      topics: _storeIds.isEmpty
          ? ['storeql/notifications/$tenantId/#']
          : [for (final storeId in _storeIds) 'storeql/notifications/$tenantId/$storeId'],
      onPayload: _emit,
    );
