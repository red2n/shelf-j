// REPL driver for the StoreQL web UI (storefront + POS + admin + platform
// console), driven against the already-running `storeql-web` Docker container.
...
const BASE_URL = process.env.STOREQL_WEB_URL || 'http://localhost:8088';
const SHOT_DIR = process.env.SCREENSHOT_DIR || '/tmp/storeql-shots';
...
const COMMANDS = {
  ...
  async login() {
    ...
    await page.keyboard.type(process.env.PLATFORM_ADMIN_EMAIL || 'admin@storeql.dev', { delay: 15 });
...
console.log('StoreQL web driver. Commands:', Object.keys(COMMANDS).join(', '));
