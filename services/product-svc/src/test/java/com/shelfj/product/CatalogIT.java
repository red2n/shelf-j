...
/**
 * Integration test for the product catalog against real Postgres (Testcontainers): create brand → category →
 * product → variant, public browse, tenant isolation, duplicate SKU 409, delist removes from public list.
 */
@HelidonTest
class CatalogIT {
    ...
    void catalogFlowAndIsolation() {
        ...
        // variant
        Response v = post("/admin/products/" + productId + "/variants", "{\"sku\":\"RICE-5KG\"}", TENANT_A);
        assertThat(v.getStatus(), is(201));

        // duplicate sku → 409
        Response dup = post("/admin/products/" + productId + "/variants", "{\"sku\":\"RICE-5KG\"}", TENANT_A);
