...
import com.storeql.product.domain.Domain.AgeRestrictionRule;
import com.storeql.product.domain.Domain.Allergen;
import com.storeql.product.domain.Domain.BirthCutoff;
import com.storeql.product.domain.Domain.VariantAllergen;
import com.storeql.product.domain.Domain.VariantCompliance;
...
public class ComplianceRepository extends BaseOutboxRepository {
   ...
   * @param tenantId owning tenant; the first condition of the query
   */
  public List<com.storeql.product.domain.Domain.VariantMeasureSource> listMeasureSources(
      UUID tenantId) {
    return query(
        ...
        rs ->
            new com.storeql.product.domain.Domain.VariantMeasureSource(
   ...
   */
  public java.util.Map<UUID, com.storeql.product.domain.Domain.DepositContainer> depositContainers(
      UUID tenantId, java.util.Collection<UUID> variantIds) {
    if (variantIds.isEmpty()) return java.util.Map.of();
    java.util.Map<UUID, com.storeql.product.domain.Domain.DepositContainer> out =
    ...
    for (var row :
        query(
            ...
            rs ->
                new com.storeql.product.domain.Domain.DepositContainer(
