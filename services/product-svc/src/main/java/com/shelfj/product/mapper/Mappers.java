...
public final class Mappers {
    ...
    public static CategoryResponse toCategory(Category c) {
        return new CategoryResponse(c.id().toString(), c.parentId() == null ? null : c.parentId().toString(), c.name());
    }

    public static ProductResponse toProduct(Product p) {
        return new ProductResponse(p.id().toString(), p.name(), p.description(),
                p.brandId() == null ? null : p.brandId().toString(),
                p.categoryId() == null ? null : p.categoryId().toString(),
                p.status(), p.sellableOnline(), p.sellablePos());
    }

    public static VariantResponse toVariant(Variant v) {
        return new VariantResponse(v.id().toString(), v.productId().toString(), v.sku(), v.barcode(),
                v.attributes(), v.unit());
    }
}
