 ...
 * the {@link com.storeql.notification.service.Notifier} and surfaced to the user in the app (see
 * the notifications feed endpoint) — no external push. Email ({@link SmtpChannel}) and, later, SMS
 * are additional channels selected via {@code storeql.notification.channel}. {@code send} is a
 * no-op here because the persisted record IS the delivery; it logs at debug for observability.
 */
