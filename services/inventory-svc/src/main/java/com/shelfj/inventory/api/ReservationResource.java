...
/**
 * Internal reservation API (called by order-svc during checkout): hold stock, then consume on confirm or release
 * on cancel/timeout. Tenant-scoped via context (gateway forwards identity).
 */
...
public class ReservationResource {
    ...
    public Response reserve(ReserveRequest req) {
        ...
        UUID orderId = req.orderId() == null || req.orderId().isBlank() ? null : uuid(req.orderId(), "orderId");
        var r = service.reserve(tenantId, uuid(req.storeId(), "storeId"), uuid(req.variantId(), "variantId"),
                req.qty(), orderId, req.ttlSeconds());
        return Response.status(Response.Status.CREATED).entity(ApiResponse.ok(Mappers.toReservation(r))).build();
    }

    @POST @Path("/{id}/consume")
