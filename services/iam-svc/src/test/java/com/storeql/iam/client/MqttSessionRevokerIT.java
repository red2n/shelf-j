...
class MqttSessionRevokerIT {
  ...
  static {
    ...
    System.setProperty("storeql.jwt.secret", JWT_SECRET);
    System.setProperty(
        "storeql.mqtt.admin-api-url", "http://" + MQTT.host() + ":" + MQTT.apiPort());
    // Must match infra/emqx-api-key.conf's literal contents — see EmqxSupport's javadoc.
    System.setProperty("storeql.mqtt.admin-api-key", "iam-svc");
    System.setProperty("storeql.mqtt.admin-api-secret", "iam_mqtt_admin_dev_change_me");
  }
  ...
  void logoutDisconnectsTheUsersLiveMqttSession() throws Exception {
    Response reg =
        post(
            "/auth/register",
            "{\"email\":\"kick@example.com\",\"password\":\"strongpass1 for storeql\"}");
    ...
    device
        .subscribeWith()
        .topicFilter("storeql/notifications/" + tenantId + "/#")
