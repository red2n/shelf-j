...
/**
 * Integration test for the iam-svc auth flow against a real Postgres (Testcontainers): register → login → refresh
 * (with single-use rotation) → /auth/me, plus validation and duplicate-detection. Kafka/Consul disabled.
 */
@HelidonTest
class AuthIT {
    ...
    void registerLoginRefreshMe() {
        // register
        Response reg = post("/auth/register", "{\"email\":\"it-user@example.com\",\"password\":\"strongpass1\"}");
        ...
        String me = target.path("/auth/me").request()
        ...
        // login
        Response login = post("/auth/login", "{\"email\":\"it-user@example.com\",\"password\":\"strongpass1\"}");
    ...
    void wrongPasswordIs401() {
        ...
        Response bad = post("/auth/login", "{\"email\":\"pw@example.com\",\"password\":\"wrongwrong\"}");
    ...
    void duplicateRegisterIs409() {
        ...
        Response dup = post("/auth/register", "{\"email\":\"dup@example.com\",\"password\":\"strongpass1\"}");
