...
public final class KeySealer {
  ...
  public KeySealer(String secret) {
    this(secret, "storeql-signing-key-seal:");
  }
  ...
  public KeySealer(String secret, String purpose) {
    if (secret == null || secret.trim().length() < 32) {
      throw new IllegalStateException(
          "storeql.jwt.secret must be set and at least 32 characters: it seals the token signing"
              + " keys at rest");
    }
    try {
      byte[] digest =
  ...
  public byte[] open(String sealed) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "the signing key could not be unsealed: was storeql.jwt.secret changed?", e);
    }
  }
}
