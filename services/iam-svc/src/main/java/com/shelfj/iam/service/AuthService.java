...
/**
 * Core authentication logic: register, login, refresh, logout. The brain of iam-svc (golden rule #9).
 *
 * <p>On register, the user row and the {@code UserRegistered} outbox event are written in one transaction
 * (golden rule #6). Refresh tokens are opaque + rotated on every refresh; only their hash is stored.</p>
 */
@ApplicationScoped
public class AuthService {
    ...
    public TokenResponse register(String email, String password, String phone) {
        ...
        var user = new User(userId, null, User.TYPE_CUSTOMER, email, phone, hash, User.STATUS_ACTIVE, Instant.now());
        ...
        var outbox = new UserRepository.OutboxRow(
    ...
    public TokenResponse login(String email, String password) {
        User user = users.findByEmail(null, email)
                .orElseThrow(() -> ApiException.unauthorized("INVALID_CREDENTIALS", "Invalid email or password"));
        if (!User.STATUS_ACTIVE.equals(user.status()) || !passwords.verify(user.passwordHash(), password)) {
    ...
    public TokenResponse refresh(String refreshToken) {
        ...
        UUID userId = refreshTokens.validate(hash)
        ...
        User user = users.findById(userId)
                .orElseThrow(() -> ApiException.unauthorized("INVALID_REFRESH", "User no longer exists"));
        return issueTokens(user);
    }
    ...
    private TokenResponse issueTokens(User user) {
        ...
        refreshTokens.store(user.id(), Tokens.hash(refresh),
