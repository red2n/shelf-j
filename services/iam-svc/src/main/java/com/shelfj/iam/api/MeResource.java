...
/**
 * {@code GET /auth/me} — returns the current principal. iam-svc verifies the access token itself (it owns JWT),
 * reading the {@code Authorization: Bearer <token>} header directly rather than relying on gateway-forwarded headers.
 */
@Path("/auth/me")
@ApplicationScoped
...
public class MeResource {
    ...
    public ApiResponse<MeResponse> me(@HeaderParam(HttpHeaders.AUTHORIZATION) String authorization) {
        ...
        try {
            ...
            throw ApiException.unauthorized("INVALID_TOKEN", "Access token invalid or expired");
        }
