 ...
 *
 * <p>{@code POST /onboarding/tenants}: the caller has NO tenant yet — we bind the new tenant to the authenticated
 * userId (from the gateway-forwarded identity). Afterwards, {@code tenantId} comes from the JWT/context.</p>
 */
@Path("/onboarding")
@ApplicationScoped
...
public class OnboardingResource {
    ...
    public Response createStore(CreateStoreRequest req) {
        ...
        return Response.status(Response.Status.CREATED)
                .entity(ApiResponse.ok(Mappers.toStore(result.store()), ApiResponse.Meta.of(ctx.requestId())))
                .build();
    }
