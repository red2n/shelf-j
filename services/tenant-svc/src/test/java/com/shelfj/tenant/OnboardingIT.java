...
/**
 * Integration test for the tenant-svc onboarding flow against real Postgres (Testcontainers): create tenant →
 * create first store (auto DEFAULT zone) → tenant isolation → duplicate-code 409. Kafka/Consul disabled; schema
 * isolation off (the test container is private), so default schema is fine.
 */
@HelidonTest
class OnboardingIT {
    ...
    void fullOnboardingFlow() {
        ...
        Response tenantResp = post("/onboarding/tenants",
                "{\"businessName\":\"Acme\",\"country\":\"in\",\"currency\":\"inr\"}", "X-User-Id", OWNER);
        ...
        Response storeResp = post("/onboarding/stores",
                "{\"name\":\"Main\",\"code\":\"MAIN\"}", "X-Tenant-Id", tenantId);
        ...
        String zones = target.path("/admin/stores/" + storeId + "/zones").request()
                .header("X-Tenant-Id", tenantId).get(String.class);
        assertThat(zones, containsString("DEFAULT"));

        // duplicate store code → 409
        Response dup = post("/onboarding/stores", "{\"name\":\"Dup\",\"code\":\"MAIN\"}", "X-Tenant-Id", tenantId);
        assertThat(dup.getStatus(), is(409));
    }

    @Test
    void createTenantRequiresUser() {
        Response noUser = post("/onboarding/tenants",
                "{\"businessName\":\"X\",\"country\":\"in\",\"currency\":\"inr\"}", null, null);
    ...
    void tenantIsolationOnStores() {
        ...
        Response t = post("/onboarding/tenants",
                "{\"businessName\":\"IsoCo\",\"country\":\"in\",\"currency\":\"inr\"}", "X-User-Id", OWNER);
        ...
        String listB = target.path("/admin/stores").request().header("X-Tenant-Id", TENANT_B).get(String.class);
