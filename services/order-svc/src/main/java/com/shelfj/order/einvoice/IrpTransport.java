 ...
 * same way. Configured with the platform's credentials ({@code shelfj.einvoice.irp.base-url},
...
public class IrpTransport implements EInvoiceTransport {
  ...
  @Inject
  @ConfigProperty(name = "shelfj.einvoice.irp.base-url")
  Optional<String> baseUrlConfig;

  @Inject
  @ConfigProperty(name = "shelfj.einvoice.irp.auth-path", defaultValue = "/eivital/v1.04/auth")
  String authPath;

  @Inject
  @ConfigProperty(name = "shelfj.einvoice.irp.invoice-path", defaultValue = "/eicore/v1.03/Invoice")
  String invoicePath;

  @Inject
  @ConfigProperty(name = "shelfj.einvoice.irp.client-id")
  Optional<String> clientIdConfig;

  @Inject
  @ConfigProperty(name = "shelfj.einvoice.irp.client-secret")
  Optional<String> clientSecretConfig;

  /** The portal's RSA public key, base64 (X.509 SubjectPublicKeyInfo). */
  @Inject
  @ConfigProperty(name = "shelfj.einvoice.irp.public-key")
  ...
  private static PublicKey keyOf(String base64) {
    try {
      ...
      throw new IllegalArgumentException(
          "shelfj.einvoice.irp.public-key is not an RSA public key", e);
    }
  }
  ...
  public String configuration() {
    return "shelfj.einvoice.irp.base-url, client-id, client-secret and public-key, and the"
