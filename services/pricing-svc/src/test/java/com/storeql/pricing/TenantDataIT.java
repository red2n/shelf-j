...
class TenantDataIT {
  ...
  void appendOnlyStillHolds() throws Exception {
    ...
    try (var c = DriverManager.getConnection(PG.jdbcUrl(), PG.username(), PG.password())) {
      ...
      c.rollback();
      try (var st = c.createStatement()) {
        st.execute("SELECT set_config('storeql.erasing_tenant', '" + Ids.newId() + "', true)");
        SQLException other =
            org.junit.jupiter.api.Assertions.assertThrows(
                SQLException.class,
                () ->
      ...
      c.rollback();
      try (var st = c.createStatement()) {
        st.execute("SELECT set_config('storeql.erasing_tenant', '" + shop + "', true)");
        SQLException update =
            org.junit.jupiter.api.Assertions.assertThrows(
                SQLException.class,
                () ->
  ...
  void readsAreTheOwners() {
    ...
    assertThat(manifest.getString("format"), startsWith("storeql-tenant-data/1"));
