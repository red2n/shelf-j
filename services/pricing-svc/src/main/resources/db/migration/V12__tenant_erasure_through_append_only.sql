...
-- being erased, in the transaction that named it in shelfj.erasing_tenant.
CREATE OR REPLACE FUNCTION applied_prices_append_only() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF TG_OP = 'DELETE' AND OLD.tenant_id::text = current_setting('shelfj.erasing_tenant', true) THEN
        RETURN OLD;
    END IF;
