...
public class TokenCipher {
  ...
  @Inject
  @ConfigProperty(name = "storeql.mtd.token-key")
  ...
  void use(String base64Key) {
    ...
    if (raw.length != 16 && raw.length != 24 && raw.length != 32) {
      throw new IllegalArgumentException(
          "storeql.mtd.token-key must be 128, 192 or 256 bits, base64");
    }
  ...
  public String encrypt(String plain) {
    if (key == null) {
      throw new IllegalStateException("storeql.mtd.token-key is not configured");
    }
    try {
