...
import com.shelfj.customer.dto.PrivacyDtos.IntimateRequest;
import com.shelfj.customer.dto.PrivacyDtos.PublishNoticeRequest;
import com.shelfj.customer.dto.PrivacyDtos.ResolveRequest;
import com.shelfj.customer.dto.PrivacyDtos.SetSettingsRequest;
import com.shelfj.customer.mapper.PrivacyMappers;
import com.shelfj.customer.service.PrivacyService;
import com.shelfj.customer.service.PrivacyService.SettingsChange;
...
public class PrivacyResource {
  ...
  public Response intimate(@Valid IntimateRequest req) {
    ...
    if (req.customerIds() != null) {
      for (String id : req.customerIds()) only.add(com.shelfj.web.Parsing.uuid(id, "customerIds"));
    }
    Intimation i =
        svc.intimate(
            ctx.requireTenantId(),
            req.noticeId() == null || req.noticeId().isBlank()
                ? null
                : com.shelfj.web.Parsing.uuid(req.noticeId(), "noticeId"),
            req.subject(),
            req.body(),
            only,
            ctx.userId());
