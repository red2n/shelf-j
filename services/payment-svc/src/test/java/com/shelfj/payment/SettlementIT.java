...
import com.shelfj.test.Envelopes;
...
class SettlementIT {
  ...
  private UUID refund(UUID paymentId, String reference, String amount) {
    ...
    payments.createRefundGuarded(
        new RefundTender(
            ...
            "changed their mind",
            Instant.now()),
        new OutboxRow("PaymentRefunded", "shelfj.payment.payment-refunded", SHOP, id, "{}"));
    return id;
  }
  ...
  void exceptionsAreDecidedAndTheBatchSignedOff() {
    ...
    payments.createTender(
        new PaymentTender(
            ...
            Instant.now(),
            HIGH_STREET),
        new OutboxRow("PaymentCaptured", "shelfj.payment.payment-captured", SHOP, cash, "{}"));
    assertThat(
  ...
  void whatCannotBeImportedIsRefused() {
    ...
    assertThat(
        importFile(me, payout("BAD-2", "9.90", good).replace("SHELFJ", "WORLDLINE"), null).code(),
