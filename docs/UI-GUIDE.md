...
- **Account menu** (signed in): My orders, **My account**, My preferences, **Privacy & marketing**, Send feedback, Sign out, **Delete my account**. (Guest): Sign in, Send feedback.
- **My account** (`/store/account`, 12.10) — what this shop holds about the shopper and the addresses they keep here, each shop its own record. A shopper the shop has no record for yet is told so and offered *Set up my account* (the claim checkout would otherwise make on first purchase). **My details**: first name, last name, phone, editable and saved with *Save*; the email is shown and not editable, because it is the sign-in. **My addresses**: each address on one line with its type and a *Default* chip; *Add address* opens a dialog (type, two lines, city, postcode, county, country, a default switch; the required fields and the server's length limits are enforced before anything is sent) and each address has *Edit*, *Make default* and *Remove* (confirmed). The server's refusal is shown in words — a full book (ten addresses) says so. Checkout offers the book: **Use a saved address** fills the delivery form from any address in it, the default first; filing a new one stays here, where the country is asked.
