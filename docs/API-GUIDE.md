...
- `GET /admin/tenant/obligations?country=&on=` — **which laws bind this business**: the legal obligations that reach a country, directly or through a regime it belongs to (the EU), each with a stable `code` services check (such as `PRICE_REDUCTION_PRIOR_PRICE`), its `scope` (`EU` or the country), `citation`, `summary`, `effectiveFrom`, `effectiveTo` and `status` — `IN_FORCE` on the day asked about, then `UPCOMING`; anything that had ended is left out. A regime's obligation is narrowed to the country's membership, so a British business is not handed EU law made after 31 Jan 2020, and `?on=2019-06-01` shows the GDPR window ending that day. `country` defaults to the business's own (`409 TENANT_COUNTRY_MISSING` if it never declared one), `on` to today; `400 COUNTRY_INVALID` / `400 OBLIGATION_DATE_INVALID` otherwise. Reference data in tenant-svc's V9 — regimes, EU membership with its dates, and 30 obligations with citations — read by any staff role (the till obeys them) and never written through the API: the law changes by migration. Services ask it through `common-service`'s `Jurisdictions` rather than keeping a list of countries.
