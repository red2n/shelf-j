...
- **Default-deny RBAC**: role model is `PLATFORM_ADMIN` (cross-tenant), `OWNER`, `MANAGER`, `STOREKEEPER`, `CASHIER`, `CUSTOMER`. The shared `AdminAuthorizationFilter` enforces four tiers: **(1)** most of the `/admin/...` subtree — every HTTP method, *including GETs* — requires a management role (`PLATFORM_ADMIN`/`OWNER`/`MANAGER`), with bootstrap/receipt exemptions (`POST /admin/tenant` during onboarding, `POST .../receipts`); **(2)** **staff-operable admin surfaces** require any staff role (`STOREKEEPER`/`CASHIER` included): `/admin/inventory/**` (warehouse), `/admin/cash/**` (till — resource layer may still demand MANAGER+ for close/drops), plus support GETs `GET /admin/tenant`, `GET /admin/tenant/obligations`, `GET /admin/stores…`, `GET /admin/products/variants/resolve`; **(3)** `POST .../refunds` and `POST .../void` require a management role even outside `/admin/`; **(4)** any other mutating request needs at least a staff role, except a small open allowlist (identity endpoints, onboarding, `/prices/resolve*`, `/orders`, `/cart*`, `/payments/online`, `/inventory/reservations*`); **(5)** **reads default-deny too** — since SJ-D11 a non-`/admin` GET needs at least a staff role unless it is on the curated open-read allowlist (storefront catalogue and config, availability, promotions, own cart, own orders, `/auth/me`, onboarding status). A GET added tomorrow ships closed. *(This paragraph previously ended "Non-`/admin` GETs outside the staff-admin list are not gated by this filter", which stopped being true when reads were closed and was found still standing during a live verification pass.)*
...
- `POST /orders/{id}/returns` / `GET /orders/{id}/returns` — create/list returns against an order (full or partial). Only goods that were handed over come back: the order must be `FULFILLED` or `PARTIALLY_REFUNDED`, otherwise `409 ORDER_CANNOT_RETURN` (cancel a `PENDING`/`CONFIRMED` order instead); returning more than was **handed over** (not ordered — on a part-fulfilled order the rest never left the store) is `409 RETURN_QTY_EXCEEDS_PURCHASED`; a `PARTIALLY_FULFILLED` order can be returned against up to what it has handed over. A return records `createdBy`, the member of staff who took the goods back (20.11); rows from before the audit trail have none.
...
### Supplier payment runs (`/payment-runs`, 17.10)
...
- `GET /payment-runs/{id}/bank-file` — the CSV for the bank's bulk upload (`text/csv`, `Cache-Control: no-store`): `payee_name,sort_code,account_number,iban,bic,amount,currency,reference`, one payment per supplier, CRLF line endings. Every text cell is guarded against formula injection (a payee named `=SUM(A1)` is written `'=SUM(A1)`). Only for an `APPROVED` or `PAID` run (`409 PURCHASE_PAYMENT_RUN_NOT_APPROVED`, `409 PURCHASE_PAYMENT_RUN_CANCELLED`), and refused with `409 PURCHASE_PAYMENT_RUN_BANK_DETAILS_CHANGED`, naming the suppliers, when any supplier's bank details changed after the run was approved.
- `POST /payment-runs/{id}/pay` — **pay an approved run**, all in one transaction: the run `PAID`, every invoice it holds `paid` with the run's id, every credit note allocated, and `Dr 2100 Trade Creditors / Cr 1200 Bank` posted per supplier and store, dated the payment date, source `SUPPLIER_PAYMENT` with the run as `sourceRef` (a store whose credits exceed its invoices posts the other way round). `SupplierRemittanceIssued` is written per supplier with an event id derived from the run and the supplier. The state condition is inside the `UPDATE`, so twenty concurrent calls pay once and the rest are `409 PURCHASE_PAYMENT_RUN_ALREADY_PAID`; a document that changed under the run is `409 PURCHASE_PAYMENT_RUN_STALE` and nothing is written. Also refused: `PURCHASE_PAYMENT_RUN_NOT_APPROVED`, `PURCHASE_PAYMENT_RUN_CANCELLED`, `PURCHASE_PAYMENT_RUN_BANK_DETAILS_CHANGED` and `PURCHASE_PERIOD_CLOSED` for a store whose period covering the payment date is closed.
...
- `GET /admin/tenant/obligations?country=&on=` — **which laws bind this business**: the legal obligations that reach a country, directly or through a regime it belongs to (the EU), each with a stable `code` services check (such as `PRICE_REDUCTION_PRIOR_PRICE`), its `scope` (`EU` or the country), `citation`, `summary`, `effectiveFrom`, `effectiveTo` and `status` — `IN_FORCE` on the day asked about, then `UPCOMING`; anything that had ended is left out. A regime's obligation is narrowed to the country's membership, so a British business is not handed EU law made after 31 Jan 2020, and `?on=2019-06-01` shows the GDPR window ending that day. `country` defaults to the business's own (`409 TENANT_COUNTRY_MISSING` if it never declared one), `on` to today; `400 COUNTRY_INVALID` / `400 OBLIGATION_DATE_INVALID` otherwise. Reference data in tenant-svc's V9 — regimes, EU membership with its dates, and 31 obligations with citations — read by any staff role (the till obeys them) and never written through the API: the law changes by migration. Services ask it through `common-service`'s `Jurisdictions` rather than keeping a list of countries.
- `GET /admin/tenant/security-notices` · `POST /admin/tenant/security-notices/{id}/acknowledge` — **security notices** the platform has sent this business about an incident that affects it (21.15), newest first, with `acknowledged` and `acknowledgedAt`. Management only (the default tier for `/admin`): the owner or a manager answers for the business as controller. Acknowledging again returns the first acknowledgement; another business's notice is `404 SECURITY_NOTICE_NOT_FOUND`.
