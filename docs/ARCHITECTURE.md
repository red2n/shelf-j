...
- **API:** `/auth` register, login, platform-login, refresh, logout, change-password, `/auth/me`; `/auth/admin/staff-users` (POST find-or-create a staff login; `GET ?ids=` names the caller's business's staff by login email — at most 100 ids, another business's staff, customers and unknown ids left out, never refused one by one — so a screen holding only user ids shows people; OWNER, MANAGER, or a platform administrator acting for the business); `/auth/pos/sessions` start/touch/end/list + `/sweep` (idle-timeout force-expire); `/bootstrap/admin` (initial platform admin).
...
- **Tables:** `inventory_batches` (with `ownership` and `duty_status`), `bond_approvals`, `excise_duty_rates`, `bond_releases`, `yield_templates` / `yield_template_outputs`, `yield_runs` / `yield_run_outputs`, `awaiting_orders` / `awaiting_order_lines` (confirmed online orders waiting to be picked, projected from `OrderConfirmed`), `awaiting_orders_done` (a tombstone per online order cancelled or handed over in full), `pick_waves` / `pick_wave_lines` / `pick_wave_allocations` / `wave_picked_lines`, `putaway_rules` / `putaway_tasks`, `serving_relationships` / `serving_exceptions` / `transfer_proposal_runs` (depot / DC replenishment: which warehouse serves which shop, what it buys direct, the runs that propose DRAFT transfers), `crossdock_expected` (what a warehouse order still owes each shop, from purchase-svc's `CrossDockAllocationsSet`), `stock_movements` (append-only), `reservations`, `serial_numbers`, `transfer_orders`, `move_orders`, `safety_stock_params`, `abc_assignments`, `lot_genealogy`, `cycle_count_headers`, `physical_inventories`, `costing_methods`, `accounting_periods`, `kanban_cards`, `reorder_point_plans`, `picking_rules`, `fs_check_types`, `fs_monitoring_points`, `fs_check_records` / `fs_corrective_actions` / `fs_reviews` / `fs_point_status_changes` (append-only), `fs_overdue_alerts`, `recalls`, `recall_items` / `recall_batches` / `recall_batch_releases` / `recall_store_actions`, `batch_cost_adjustments` (append-only: every change to a batch's cost, what it was and what it became, and the charge that moved it — 07.x) (append-only), and more. Plus `catalog_lines_out` (item lifecycle: the variants product-svc has discontinued or delisted, projected from its lifecycle events, so low-stock and planning leave them alone) and `shelf_targets` / `shelf_target_fixtures` (07.17: what each bay holds, projected from product-svc's published planograms, with the version that makes a re-delivered older layout a no-op).
- **Events:** publishes `StockReceived`, `StockReserved`, `StockReleased`, `StockDeducted`, `StockAdjusted`, `StockBelowThreshold`, `ReplenishmentSuggested`, `TransferOrderShipped/Received`, `CycleCountAdjusted`, `KanbanTriggered`, `FoodSafetyCheckFailed`, `FoodSafetyCheckOverdue`, `RecallOpened`, `RecallSaleAffected` (05.10: one per order a recall reached, found from the sale movements that drew on the recalled batches), and more; consumes `GoodsReceived` and `LandedCostApplied` / `LandedCostReversed` (purchase-svc; a landed charge lifts the cost of the receipt's batches, found through their RECEIVE movements, and a receipt not yet booked is redelivered rather than lost), `OrderFulfilled`/`OrderReturned`/`OrderCancelled` (order-svc), `ShelfCapacityPublished`/`FixtureRetired` (product-svc). `ConsignmentStockSold` (a sale drawn from a supplier's consignment batch, at the order's price; purchase-svc owes the supplier for it). Consumes `VariantSourcingChanged` (purchase-svc): a dropship variant is available with nothing on the shelf, its hold draws nothing, its sale deducts nothing. Publishes `DutyReleased` (a release from a bonded warehouse: the quantity, the duty per unit and the duty owed, in the home currency) and `YieldRecorded` (a breakdown: the primal consumed, each cut made as a batch at its apportioned cost, the loss against expected), beside the `StockAdjusted` and `StockReceived` a breakdown also announces. Consumes `OrderConfirmed` (an online order for pickup or delivery waits at its store, from the event's `occurredAt`; its own consumer group, latest offset) and publishes `WavePicked` when a wave completes: the picked batches are deducted as SALE movements against each order on the wave's transaction (only what each order still waits for; a consignment draw announces `ConsignmentStockSold`), the holds reduced, and order-svc hands the orders over once per event on the handover's own transaction; the `OrderFulfilled` that follows (now with `outstandingQty` per line and the order's `status`) deducts only what the wave did not pick, records the line's revenue once, and sets the waiting list from the outstanding figures; a cancelled or fulfilled order leaves a tombstone (`awaiting_orders_done`). A warehouse serves shops (`serving_relationships`; the store types are tenant-svc's, read through `TenantProfiles`): its replenishment run proposes DRAFT transfers to the shops at or below their reorder point, shared fairly when it is short, released by a person; `TransferOrderShipped` / `TransferOrderReceived` carry their lines; `GET /admin/inventory/network/sourcing` tells purchase-svc what a shop buys direct and what a warehouse's shops need. Cross-docking: consumes `CrossDockAllocationsSet` (purchase-svc, a snapshot per order) and at a warehouse's `GoodsReceived` sends the owed lines straight across the dock as PENDING `CROSSDOCK` transfers shipping from the batch the delivery made. Directed putaway hooks every batch insert: a batch with no zone is placed by `putaway_rules` (product, else store default) or raised as a `putaway_tasks` row, read and answered with its batch's `batchNo` (joined in the same tenant-scoped statement) so the list names the batch as printed.
...
- **Ship-from-store and dark-store picking** ([intent](../intent/ship-from-store-and-dark-store-picking.md)): FULFILLED is picked and packed; the handover after it — `order_handovers`, one per order, append-only: DISPATCHED to a carrier or COLLECTED at the counter — is `POST /orders/{id}/dispatch` / `collect`, publishing `OrderDispatched` / `OrderCollected` (notification-svc tells the shopper; webhooks). `OrderFulfilled` names the buyer so a pickup picked in full is told *ready for collection*. A `DARK_STORE` (tenant-svc's type, read through `TenantProfiles.Stores.isDark`) takes no pickup and no till sale.
- **Substitutions for out-of-stock online lines** ([intent](../intent/substitutions-for-out-of-stock-online-lines.md)): `orders.allow_substitutions` is the shopper's choice at checkout (on unless turned off); `order_items.short_qty` is what a line will never hand over (`outstanding = qty − fulfilled − short`, the charge and VAT pro rata to what stands) and `substitutes_item_id` which line a substitute stands in for; `order_line_adjustments` is append-only. `POST /orders/{id}/lines/{variantId}/short` and `…/substitute` run on one transaction (`OrderRepository.adjustLine`): the substitute priced by pricing-svc at the store and capped at the original's gross unit price by pure `SubstitutePrice`, inserted already picked, the totals recomputed, the order FULFILLED once every line is picked or closed, `OrderLineShortClosed` / `OrderLineSubstituted` (+ `OrderFulfilled` for the substitute) in the outbox. payment-svc refunds the difference (`PaymentRefunded.kind = ORDER_ADJUSTMENT`, which `applyRefundOnce` records without moving the status), inventory-svc gives the hold back and shortens the waiting line (`InventoryRepository.lineClosedOnce`), notification-svc tells the shopper.
- **Order orchestration** ([intent](../intent/order-orchestration-and-split-fulfilment.md)): an online delivery the delivery-area store cannot fill alone is routed (`domain/Routing`, pure; `service/OrderRouter` reads inventory-svc's stock by store and tenant-svc's shops and coordinates) and placed as an **order group** — `order_groups` plus one child order per shop (`orders.group_id`, `group_part`), each an ordinary one-store order with its own holds and events, so no consumer changes; `OrderPlaced` names the `groupId`. `GET /order-groups/{id}`. payment-svc takes one payment for the group — a tender per part.
...
- **API:** `/payments` capture, online, by-order, refunds; `/admin/cash/till-sessions` open/drops/x-report/close, and `GET /current?storeId=` — the caller's own open till at a store (`404 TILL_SESSION_NOT_OPEN` when there is none; a colleague's, a closed one or another business's is never current), which a till screen resumes after a reload; `/admin/cash` movements (pay-in/pay-out), z-report.
...
- **Events:** publishes `PurchaseOrderCreated`, `GoodsReceived` (with each line's `costPrice`, the order's price), `LandedCostApplied` / `LandedCostReversed` (07.x: what each unit's cost rises or falls by, keyed on the charge), `ReturnedToVendor`, `IntercompanyInvoiceRaised`, `SupplierInvoiceCaptured`; consumes `OrderConfirmed`, `PaymentCaptured` and `PaymentRefunded` (17.7: the sale, its tenders and refunds on the nominal ledger) and `LoyaltyEarned/Redeemed/Adjusted` and `GiftCardLoaded` (17.11: deferred revenue for loyalty points and gift card breakage), each posted once. Consumes `ConsignmentStockSold` (inventory-svc): each sale recorded once in `consignment_sales`, Dr Purchases - Consignment / Cr Trade Creditors; `consignment_settlements` gather a period's sales into the statement the supplier invoices against. `GoodsReceived` carries `ownership` and `supplierId`, and a consignment receipt posts nothing. Dropship: `dropship_arrangements` (one live per variant) publish `VariantSourcingChanged`; `OrderConfirmed` (now with the fulfilment type and delivery address) raises one DRAFT `source: DROPSHIP` order per supplier, shipped to the customer, never received — delivered, Dr Purchases - Dropship / Cr GRNI. Bonded stock: a purchase order carries `duty_status` and its `GoodsReceived` says so; consumes `DutyReleased` (inventory-svc), each release recorded once in `duty_releases`, Dr Excise Duty / Cr Excise Duty Payable. Supplier scorecards: `suppliers.lead_time_days` (the quote), `purchase_orders.submitted_at` (when the order went out) and `supplier_deliveries` (one fact per goods receipt, measured on the receipt's transaction: lead days, days late against the order's date or the quote, complete, quantity); `SupplierPerformanceRepository` aggregates a period's deliveries, fill (finished orders), returns and invoice matches per supplier and `SupplierScorecard` (pure) weighs them 40/30/20/10 over what is known into a score and grade. RFQs: `rfqs` (a tenant series `RFQ-000001`, DRAFT → ISSUED → AWARDED | CANCELLED), `rfq_lines`, `rfq_suppliers` (INVITED | QUOTED | DECLINED with the quote's terms), `rfq_quote_lines`, `rfq_awards` (a line once, on the order it raised); `Rfq.compare` (pure) reads every quote at home through `FxRates` — the lowest per line, totals and a rank over complete translatable bids — and `RfqService.award` raises one DRAFT `source: RFQ` order per supplier at the quoted prices through the same repository calls a proposal or a dropship uses.
...
- **API:** `/customers` CRUD (+anonymize-on-delete) and `GET /customers?q=` — the staff search, by first, last or full name, email or phone, ignoring case, bound as an escaped pattern within the caller's tenant, a phone-shaped `q` also matched on its digits alone, on the same cursor as the list; addresses CRUD; `/{id}/loyalty` earn/redeem/adjust/ledger; `/{id}/store-credit` issue/redeem.
...
- **API:** inventory — `/admin/reports/inventory/on-hand`, `/supply-demand` (nets against open in-transit supply), `/movement-stats` (bucketed daily/weekly/monthly); sales — `/admin/reports/sales/labour` (what each day took against what its hours cost), `/admin/reports/sales/summary` (gross/refunded/net revenue + order count per currency), `/admin/reports/sales/by-day` (daily revenue buckets) — every sales report, categories and labour against takings included, leaves out a till sale voided after the fact.
- **Tables:** `inventory_projection`, `movement_events`, `open_supply_lines`, `sales_facts` (with `voided_at`: a voided sale's fact is marked, never deleted), `sales_voids` (every void heard, one per order, so a void that arrives before its sale still marks it), `labour_facts` (what a store's hours cost, per time entry, projected from tenant-svc — with no person on the row, so pay stays where it is kept), `outbox` (21.14: the evidence of a departed business's erasure).
- **Events:** consumes `StockReceived`, `StockDeducted`, `StockAdjusted`, `TransferOrderShipped/Received` (stock projections), `OrderConfirmed` (the sale, and since 19.x its `lines`), `PaymentRefunded` (sales projection, net of refunds), `OrderVoided` (nets a voided till sale out of every sales report, deduped per event, scoped by the event's tenant) and `ProductCategorised`, `VariantCreated` (the catalogue projection sales by category is read against); publishes nothing.
...
| `ConsignmentStockSold` | inventory-svc | purchase-svc |
| `DutyReleased` | inventory-svc | purchase-svc (Dr Excise Duty / Cr Excise Duty Payable, the day the goods left bond) |
| `YieldRecorded` | inventory-svc | (a report that wants yield by store and template; the primal's `StockAdjusted` and each cut's `StockReceived` go with it, so the stock projections stay right) |
| `WavePicked` | inventory-svc | order-svc (fulfils each order named for the quantities picked, once; `StockDeducted` per order goes with it) |
| `OrderDispatched` | order-svc | notification-svc (tells the shopper their delivery is on its way, with the carrier and reference), webhooks |
| `OrderCollected` | order-svc | webhooks (a picked pickup order was collected at the counter) |
| `OrderLineShortClosed` | order-svc | payment-svc (refunds the amount, `kind: ORDER_ADJUSTMENT`), inventory-svc (gives that much of the hold back, shortens the waiting line), notification-svc (`ORDER_LINE_SHORT`), webhooks |
| `OrderLineSubstituted` | order-svc | payment-svc (the difference, `kind: ORDER_ADJUSTMENT`), inventory-svc (the original's hold back; the substitute's draw rides the `OrderFulfilled` beside it), notification-svc (`ORDER_LINE_SUBSTITUTED`), webhooks |
| `OrderConfirmed` | order-svc | purchase-svc (sales ledger, dropship), inventory-svc (an online order for pickup or delivery waits to be picked at its store) |
| `VariantSourcingChanged` | purchase-svc | inventory-svc |
...
| `OrderVoided` | order-svc | inventory-svc (receives back what the voided till sale took, net of anything already returned), reporting-svc (nets voided sales out of sales reports: the sale's fact marked `voided_at`, never deleted — gone from the summary, the days, the categories and labour against takings; a void heard before its sale is kept in `sales_voids` and marks it when it lands) |
...
Browse (public catalog) → add to cart → view live price (pricing-svc) → POST /orders (Idempotency-Key)
  ...
  a delivery the delivery-area store cannot fill alone (order orchestration):
  order-svc reads stock by store (inventory-svc) + shops and coordinates (tenant-svc)
    → Routing: area store first, then the nearest shop holding the rest, else the fewest shops
    → one child order per shop under an order group, each held at its own shop (all or nothing)
  client pays the group once: /payments/online {groupId} → a tender + PaymentCaptured per part
    → each part confirms, is picked, fulfilled and refunded at its own shop as any order
```
...
**POS sale (cashier):**
```
Cashier opens POS session (iam-svc) → opens a till, or resumes the one they left open at the store
  (payment-svc; GET /admin/cash/till-sessions/current) → scans barcode (product-svc)
...
| **POS** | `/pos/*` | cashier or admin | Build a sale (scan/add items), park/resume held sales, split-tender across cash/card/UPI/wallet/gift-card/store-credit, till open/resume/close, offline sales pending sync (reachable clocked out), print receipts (an "email receipt" is currently only logged, not sent). |
...
| **Platform console** | `/platform/*` | platform admin (separate login) | Overview (the gateway's health, as it answered), tenants (activate/deactivate, assisted onboarding), plans, billing (receivables named by business), security incidents (`/platform/security/:id` for one). |
...
- **Names, not ids:** screens read the name behind an id rather than show it — a member of staff's login email from iam-svc's `GET /auth/admin/staff-users?ids=` (`StaffNameCache`, each person asked about once per screen), a product and SKU from product-svc, a store from tenant-svc's list, the business from `/storefront/stores`' `businessName` — with the end of the id only while a name is unknown. The Customers search asks customer-svc (`GET /customers?q=`), debounced, a cursor page at a time (`CustomerSearchNotifier`); the POS Cash screen asks payment-svc for the cashier's open till (`GET /admin/cash/till-sessions/current`) instead of remembering it on the device.
