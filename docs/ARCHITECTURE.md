...
- **API:** `/orders` create/confirm/cancel/fulfil/void/returns; `/layaways` create/deposit/complete/cancel; `/gift-cards` issue/reload/redeem/transactions; `/pos/parked-sales`, `/pos/no-sale`; `/admin/special-orders`; `/admin/pos-log`; `/admin/pos/stock-positions`; `/admin/orders/{id}/receipts` (e-journal, print/email). `/admin/orders/{id}/invoice(s)`, `/admin/returns/{id}/credit-note`, `/admin/sales-invoices` (18.9: EN 16931 invoices and credit notes to VAT-registered buyers, issued in the background of a sale and a return, numbered gaplessly, downloaded as UBL, CII, Factur-X or India's INV-01).
- **Tables:** `orders`, `order_items`, `order_status_history` (append-only), `returns`, `layaways`, `gift_cards`, `gift_card_transactions`, `parked_sales`, `special_orders`, `pos_log_entries`, `order_receipts`, `idempotency_keys`, `receipt_series`, `fiscal_receipts` (append-only, hash-chained, stamped by the store's fiscal regime), `fiscal_store_settings`, `tse_devices`, `age_verifications` (append-only), `customer_erasures`, `sales_invoice_series`, `sales_invoices` (18.9: the UBL as issued, never updated; `order_items.vat_code`/`vat_rate` from the quote).
