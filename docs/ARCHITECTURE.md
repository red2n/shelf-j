...
- **IDs:** UUIDv7 is the only uuid version StoreQL stores. v7 ids lead with a millisecond timestamp, so new rows append to the right-hand edge of each index rather than splitting random pages.
