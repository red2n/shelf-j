...
FUNCTIONAL=(
  ...
  privacy-flow compliance-flow purchase-crud vat-return-flow markdown-flow audit-trail storefront-account notification-channels ledger-flow role-model promotion-scoping payment-run-flow tenant-defaults sales-posting security-incidents recall-buyers retention-flow web-shell deferred-revenue bank-files tenant-export einvoice-inbound einvoice-outbound einvoice-transport dpdp-flow cash-limit-flow deposit-return-flow item-lifecycle-flow script-integrity-flow password-policy-flow problem-details-flow token-signing-flow mfa-flow chargeback-flow settlement-flow plans-flow tenant-limits-flow health-probes billing-flow dunning-flow statutory-returns gs1-scan terminal-payments merchandising-range ereporting-flow workforce-flow commission-flow store-tasks-flow broadcast-flow landed-cost-flow sso-flow message-templates-flow uuid-v7-flow gross-margin stock-provenance-flow usage-metering-flow
