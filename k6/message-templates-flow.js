...
export default function ({ tenant, rival, store, variantId, storekeeper, cashier, polish, plain, other }) {
  ...
  truthy('[+] twelve messages, to customers, staff and suppliers — the platform\'s two billing notices among them', list(catalogue).length === 12 && ['CUSTOMER', 'STAFF', 'SUPPLIER'].every((a) => list(catalogue).some((m) => m.audience === a)), list(catalogue).map((m) => m.type));
