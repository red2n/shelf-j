 ...
 * shelfj.clients.<service>.url} when that is set — for a deployment without discovery, and for
...
public final class ServiceReader {
  ...
  /**
   * A service's address as {@code shelfj.clients.<service>.url} sets it; empty when it is not set,
   * and discovery decides.
   */
  public static Optional<String> configuredUrl(String service) {
    return ConfigProvider.getConfig()
        .getOptionalValue("shelfj.clients." + service + ".url", String.class)
        .filter(u -> !u.isBlank());
  }
