 ...
 * STOREQL_ADVERTISE_HOST} (set per container in compose), defaulting to localhost for host-run dev.
 */
@ApplicationScoped
public class ConsulRegistrar {
  ...
  /**
   * @return {@code STOREQL_ADVERTISE_HOST} if set and non-blank (the per-container hostname/IP
   *     compose/k8s assigns), otherwise {@code "localhost"} for host-run dev
   */
  private static String advertiseHost() {
    String host = System.getenv("STOREQL_ADVERTISE_HOST");
