 ...
 *
 * <p>Activated only when {@code storeql.config.url} is set (env or system property); otherwise it
 * is an empty no-op, so local dev and tests run unchanged. If config-svc is unreachable, or has no
 ...
 * from env / a secret store. The one required credential, {@code storeql.config.token}, is read
 * from the environment, not fetched.
 */
public final class ConfigServiceConfigSource implements ConfigSource {
  ...
  private static final String NAME = "storeql-config-service";
  ...
  public ConfigServiceConfigSource() {
    this.ordinal = parseInt(bootstrap("storeql.config.ordinal"), DEFAULT_ORDINAL);
    this.properties = load();
  }

  /**
   * @return the fetched config key/value map, or {@link Map#of()} if {@code storeql.config.url} is
   *     unset, {@code storeql.service.name} is unknown, config-svc returns 404/non-200, or
   *     config-svc is unreachable — every failure mode degrades to "no central config" rather than
   *     throwing, per the class doc
   */
  private Map<String, String> load() {
    String url = bootstrap("storeql.config.url");
    if (url == null || url.isBlank()) {
      return Map.of(); // not configured — no-op (local dev / tests)
    }
    String service = bootstrap("storeql.service.name");
    if (service == null || service.isBlank()) {
      LOG.log(
          Level.WARNING,
          "storeql.config.url is set but storeql.service.name is unknown — skipping central config");
      return Map.of();
    }
    String profile = orDefault(bootstrap("storeql.config.profile"), "default");
    String token = orDefault(bootstrap("storeql.config.token"), "");
   ...
   * (source of {@code storeql.service.name}). Returns {@code null} if unset everywhere.
   *
   * @param key the dotted MicroProfile Config key, e.g. {@code "storeql.config.url"}
   ...
   *     storeql.config.ordinal}) — higher wins ties over the classpath properties file (100) but
