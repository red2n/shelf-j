...
public abstract class BaseServiceConfig implements ServiceSettings {
   ...
   * point at PgBouncer. Property: {@code storeql.db.url}. Default: local-dev Postgres on {@code
   * localhost:5432/storeql}. Misconfiguring this surfaces as {@link HealthChecks.DatabaseReadiness}
   * reporting DOWN, not a startup crash.
   */
  @Inject
  @ConfigProperty(
      name = "storeql.db.url",
      defaultValue = "jdbc:postgresql://localhost:5432/storeql")
  String dbUrl;

  /**
   * JDBC URL {@link FlywayRunner} migrates against — see {@link ServiceSettings#dbMigrationUrl()}
   * for why this can differ from {@link #dbUrl}. Property: {@code storeql.db.migration-url}.
  ...
  @Inject
  @ConfigProperty(
      name = "storeql.db.migration-url",
      defaultValue = "jdbc:postgresql://localhost:5432/storeql")
  String dbMigrationUrl;

  /** Postgres user for both the app pool and Flyway. Property: {@code storeql.db.user}. */
  @Inject
  @ConfigProperty(name = "storeql.db.user", defaultValue = "storeql")
  String dbUser;

  /**
   * Postgres password for both the app pool and Flyway. Property: {@code storeql.db.password}. The
   ...
   */
  @Inject
  @ConfigProperty(name = "storeql.db.password", defaultValue = "storeql_dev_change_me")
  String dbPassword;

  /**
   * Max HikariCP pool size for this service instance ({@link DataSourceProducer}). Property: {@code
   * storeql.db.pool-max-size}. Too high risks exhausting Postgres's {@code max_connections} across
   * replicas; too low serializes concurrent requests waiting on a connection.
   */
  @Inject
  @ConfigProperty(name = "storeql.db.pool-max-size", defaultValue = "10")
  int dbPoolMaxSize;

  /** Consul agent host used by {@link ConsulRegistrar}. Property: {@code storeql.consul.host}. */
  @Inject
  @ConfigProperty(name = "storeql.consul.host", defaultValue = "localhost")
  String consulHost;

  /** Consul agent port used by {@link ConsulRegistrar}. Property: {@code storeql.consul.port}. */
  @Inject
   ...
   * storeql.consul.enabled}. Set {@code false} only for isolated tests that don't need discovery —
   * production and normal dev always register (golden rule #4).
   */
  @Inject
  @ConfigProperty(name = "storeql.consul.enabled", defaultValue = "true")
  boolean consulEnabled;

  /**
   * Whether Kafka consumers ({@link BaseKafkaConsumer}) and the {@link OutboxPublisher} start.
   * Property: {@code storeql.kafka.enabled}. When {@code false}, both no-op at startup instead of
  ...
  /**
   * Kafka bootstrap servers, e.g. {@code "kafka:9092"}. Property: {@code storeql.kafka.bootstrap}.
   ...
   * storeql.outbox.poll-seconds}. Lower values reduce event-publish latency at the cost of more
   * frequent {@code FOR UPDATE SKIP LOCKED} polling queries.
   */
  @Inject
  @ConfigProperty(name = "storeql.outbox.poll-seconds", defaultValue = "5")
