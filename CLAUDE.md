...
- **UI theme (23 Sep 2026):** `frontends/storeql-app/lib/core/theme.dart` mirrors the **Claude design system** (artifact `2507697b-5dab-4a2e-8ac9-2a27b0d8b4fb`, brand book at `project/README.md`) one-to-one — every Material 3 role written out for both brightnesses, "warm and quiet". Build screens from the roles and the tokens (`AppRadius`, `AppSpacing`, `context.status.*` with its `*Container` pairs, `context.channelAccent` in POS); never a hard-coded colour or `BorderRadius.circular(<number>)` in a feature (the four exceptions are listed in [docs/UI-GUIDE.md](docs/UI-GUIDE.md) §7.1). `test/core/theme_contrast_test.dart` measures every text pair at 4.5:1 in both themes and fails a palette change that breaks one. Shared states: `EmptyState`, `ErrorView`, `LoadingView`, `Skeleton`. **Adaptive standard (25 Sep 2026, [UI-GUIDE §7.2](docs/UI-GUIDE.md)):** every screen decides layout from the width it gets (`AppBreakpoints.classOf`, never ad-hoc 700/880), insets by `context.pageGutter` (16 phone / 24 wider), caps with `ContentBounds`/`ContentBounds.form`, and builds from `lib/shared/widgets/` — `PageHeader`, `AdaptiveActions`, `AdaptiveFilters`, `ScrollableTable`, `BottomActionBar`, `showAdaptiveSheet`, `StatusBadge`; words not codes (`status_labels.dart`), money through `AppFormat.money`, dates through `AppFormat.date`/`dateTime`, people and things by name (`staffLoginsProvider`, `variantLabelsProvider`; `shortRef` only while a name is unknown). The design system's screen cards are the review record: a UI change that makes a card untrue updates the card.
...
- **Substitutions for out-of-stock online lines** ([intent](intent/substitutions-for-out-of-stock-online-lines.md)): `orders.allow_substitutions` is the shopper's choice at checkout (on unless turned off) and is read **from the order**, never from the request; `order_items.short_qty` is what a line will never hand over (`outstanding = qty − fulfilled_qty − short_qty`; the charge, VAT and deposit pro rata to what stands), `substitutes_item_id` which line a substitute stands in for, `order_line_adjustments` append-only. `POST /orders/{id}/lines/{variantId}/short` and `…/substitute` (staff at the order's store, `Idempotency-Key`) go through `OrderRepository.adjustLine` on one transaction: a substitute is priced by pricing-svc at the order's store and **charged at no more than the original's gross unit price** (pure `SubstitutePrice`: the lower of the two, its own VAT rate inside a capped price, the difference refunded — never more, never a re-authorisation), inserted already picked so its `OrderFulfilled` deducts and records revenue as any line's; **FULFILLED once every line is picked or closed**. `OrderLineShortClosed` / `OrderLineSubstituted` carry `refundAmount`: payment-svc refunds it once per event with `PaymentRefunded.kind = ORDER_ADJUSTMENT`, which `applyRefundOnce` records **without moving the status**; inventory-svc's `lineClosedOnce` gives that much of the hold back (shrink, or release + `StockReleased`) and shortens the waiting line on one transaction; notification-svc sends `ORDER_LINE_SHORT` / `ORDER_LINE_SUBSTITUTED`. A refused substitute is a return at handover; the picker decides, the platform suggests (product-svc `SUBSTITUTE` relationships + the shelf) and never substitutes on its own.
...
- **Cross-docking** ([intent](intent/cross-docking.md)): purchase-svc owns a warehouse order's line allocations (`purchase_order_line_allocations`, set while DRAFT, only to served shops, never consignment or bonded) and announces them as a **whole snapshot per order** (`CrossDockAllocationsSet`, keyed by the order; empty on reject/cancel/close-short) — never deltas. inventory-svc owns what is still owed (`crossdock_expected`), counts it inbound at the shop, and at the warehouse's `GoodsReceived` handles the owed lines on **one receipt-level transaction** (`CrossDockService.receive`): a crossing batch that is never put away, one PENDING `CROSSDOCK` transfer per shop whose lines ship that batch first (`transfer_order_lines.source_batch_id`), a short line shared by pure `DcReplenishment.share`.
- **Order orchestration and split fulfilment** ([intent](intent/order-orchestration-and-split-fulfilment.md)): an ONLINE DELIVERY order the delivery-area store cannot fill alone is routed by pure `Routing` (area store first, the nearest shop holding the rest, else the fewest shops; a line split only when it must be) over **shops only** — type STORE, trading, the caller may act there; **a warehouse never fills a shopper's order** — through `OrderRouter` (inventory-svc `GET /admin/inventory/network/stock`, tenant-svc stores and coordinates via `TenantProfiles.Stores.where`). Several shops → an `order_groups` row and one ordinary child order per shop (`orders.group_id`/`group_part`), holds all-or-nothing, money shared by pure `OrderSplit` (last part takes the rounding); every consumer sees one-store orders, and only `OrderPlaced` names the `groupId`. **An unreadable stock answer never refuses a sale** (placed at the area store as before). A group is **paid once**: `POST /payments/online {groupId}` records a tender and a `PaymentCaptured` per part; a part is never paid alone (`PAYMENT_ORDER_IN_GROUP`).
- **Ship-from-store and dark-store picking** ([intent](intent/ship-from-store-and-dark-store-picking.md)): **FULFILLED means picked and packed** (what a completed wave leaves; the sale recognised, the stock gone) and the **handover is a second, recorded step** — `order_handovers`, one per order, append-only, `POST /orders/{id}/dispatch {carrier, reference?, parcels?}` for an online DELIVERY, `POST /orders/{id}/collect {collectedBy?}` for an online PICKUP, by any staff assigned to the order's store; never on a till sale, never before FULFILLED (a part-picked delivery waits), never twice. `orders.status` is not changed by it, so every consumer keeps one meaning of FULFILLED; `OrderDispatched`/`OrderCollected` are the new events, and `OrderFulfilled` names the buyer so notification-svc says *ready for collection* (pickup, FULFILLED, ONLINE only) and *on its way*. A **`DARK_STORE`** is tenant-svc's third store type: a shop to routing, depot and cross-dock (`isWarehouse` false), delivery-only on the storefront (`pickupOffered: false`), `409 ORDER_PICKUP_NOT_OFFERED` / `ORDER_NO_TILL_AT_DARK_STORE` in order-svc (via `TenantProfiles.Stores.isDark`, fail-open when unreadable), `409 POS_STORE_HAS_NO_TILL` in iam-svc (its `store_types` projection from `StoreStatusChanged.type`).
